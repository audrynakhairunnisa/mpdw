...
```{r}
library(rio) 
dt <- import("https://raw.githubusercontent.com/audrynakhairunnisa/mpdw/main/Pertemuan%201/Air%20Quality%20Historical%20%28Data%20Individu%29.xlsx")
...
```{r}
ts.plot(dt.ts, xlab="Time Period ", ylab="Reading", 
        main = "Time Series Plot")
points(dt.ts)

#menyimpan plot
dev.copy(png, "plot_dt.png")
dev.off()
```
...
# ubah ke time series
train_ma.ts <- ts(training_ma$`PM2.5 (ug/m3)`)
test_ma.ts  <- ts(testing_ma$`PM2.5 (ug/m3)`)
```
...
ggplot() + 
  geom_line(
    data = training_ma,
    aes(x = Tanggal, y = `PM2.5 (ug/m3)`, col = "Data Latih")
  ) +
  geom_line(
    data = testing_ma,
    aes(x = Tanggal, y = `PM2.5 (ug/m3)`, col = "Data Uji")
  ) +
  labs(
    x = "Periode Waktu",
    y = "PM2.5 (ug/m3)",
...
Dalam hal ini nilai MAPE data latih pada metode pemulusan SMA sekitar 19,64%, nilai ini dapat dikategorikan sebagai nilai akurasi yang baik. Selanjutnya dilakukan perhitungan nilai MAPE data uji pada metode pemulusan SMA.
...
Perhitungan akurasi menggunakan data latih menghasilkan nilai MAPE sebesar 19,53%. Berdasarkan nilai tersebut, hasil peramalan dapat dikategorikan memiliki tingkat akurasi yang baik.
...
Perhitungan akurasi pada data latih menggunakan nilai MAPE menghasilkan nilai MAPE yang lebih dari 10% (22,15%) sehingga dikategorikan baik. Selanjutnya, perhitungan nilai akurasi dilakukan pada data uji.
...
Perhitungan akurasi menggunakan data latih menghasilkan nilai MAPE yang lebih dari 10% (24,78%) sehingga nilai akurasi ini dapat dikategorikan sebagai baik.

Pada data latih maupun data uji, metode SMA lebih baik dibandingkan dengan metode DMA.
...
# ubah ke time series
train.ts <- ts(training$`PM2.5 (ug/m3)`)
test.ts  <- ts(testing$`PM2.5 (ug/m3)`)
```
...
resid1 <- training$`PM2.5 (ug/m3)` - ramalan1$fitted
head(resid1)
...
MAPE.1 <- sum(
  abs(sisaan1[2:length(train.ts)] / train.ts[2:length(train.ts)]) * 100
) / length(train.ts)
MAPE.1
...
resid2 <- training$`PM2.5 (ug/m3)` - ramalan2$fitted
head(resid2)
...
MAPE.2 <- sum(
  abs(sisaan2[2:length(train.ts)] / train.ts[2:length(train.ts)]) * 100
) / length(train.ts)
MAPE.2
...
Berdasarkan nilai SSE, MSE, RMSE, dan MAPE di antara kedua parameter, nilai parameter $\lambda=0,7$ menghasilkan akurasi yang lebih baik dibanding $\lambda=0,2$ . Hal ini dilihat dari nilai masing-masing ukuran akurasi yang lebih kecil. Berdasarkan nilai MAPE-nya, hasil ini dapat dikategorikan sebagai peramalan baik.
...
e1 <- as.numeric(ramalan1$mean)[1:n_test] -
      as.numeric(testing$`PM2.5 (ug/m3)`)
...
```{r}
ramalan1 <- ses(train.ts, alpha = 0.2, h = nrow(testing))
ramalan2 <- ses(train.ts, alpha = 0.7, h = nrow(testing))
ramalanopt <- ses(train.ts, h = nrow(testing))
```


```{r}
accuracy(ramalan1, testing$`PM2.5 (ug/m3)`)
accuracy(ramalan2, testing$`PM2.5 (ug/m3)`)
accuracy(ramalanopt, testing$`PM2.5 (ug/m3)`)
```
...
selisihdes1 <- ramalandes1$mean - testing$`PM2.5 (ug/m3)`
selisihdes1

SSEtestingdes1 <- sum(selisihdes1^2)
MSEtestingdes1 <- SSEtestingdes1 / length(testing$`PM2.5 (ug/m3)`)
MAPEtestingdes1 <- sum(
  abs(selisihdes1 / testing$`PM2.5 (ug/m3)`) * 100
) / length(testing$`PM2.5 (ug/m3)`)


selisihdes2 <- ramalandes2$mean - testing$`PM2.5 (ug/m3)`
selisihdes2

SSEtestingdes2 <- sum(selisihdes2^2)
MSEtestingdes2 <- SSEtestingdes2 / length(testing$`PM2.5 (ug/m3)`)
MAPEtestingdes2 <- sum(
  abs(selisihdes2 / testing$`PM2.5 (ug/m3)`) * 100
) / length(testing$`PM2.5 (ug/m3)`)


selisihdesopt <- ramalandesopt$mean - testing$`PM2.5 (ug/m3)`
selisihdesopt

SSEtestingdesopt <- sum(selisihdesopt^2)
MSEtestingdesopt <- SSEtestingdesopt / length(testing$`PM2.5 (ug/m3)`)
MAPEtestingdesopt <- sum(
  abs(selisihdesopt / testing$`PM2.5 (ug/m3)`) * 100
) / length(testing$`PM2.5 (ug/m3)`)


akurasitestingdes <- matrix(
  c(
    SSEtestingdes1, MSEtestingdes1, MAPEtestingdes1,
...
training.ts <- ts(training$`PM2.5 (ug/m3)`, frequency = 7)
testing.ts <- ts(testing$`PM2.5 (ug/m3)`, frequency = 7)
```
...
# Forecast Model 1 berupa data frame, ambil kolom pertama
pred1 <- as.numeric(forecast1[[1]])

# Forecast Model 1 Optimal berupa time series
pred1.opt <- as.numeric(forecast1.opt)

# Data aktual testing
aktual <- as.numeric(testing$`PM2.5 (ug/m3)`)

# Samakan panjang
pred1 <- pred1[1:length(aktual)]
pred1.opt <- pred1.opt[1:length(aktual)]

# Model 1
selisih1 <- pred1 - aktual

SSEtesting1 <- sum(selisih1^2)
MSEtesting1 <- SSEtesting1 / length(aktual)

# Model 1 Optimal
selisih1.opt <- pred1.opt - aktual
...
Berdasarkan hasil akurasi data uji, Model 2 menghasilkan kinerja yang lebih baik dibandingkan Model 1. Hal ini ditunjukkan oleh nilai MSE Model 2 sebesar 514,3243 yang lebih kecil dibandingkan Model 1 sebesar 652,1631. Oleh karena itu, Model 2 dipilih sebagai model terbaik karena menghasilkan kesalahan peramalan yang lebih kecil.


#Kesimpulan 


Berdasarkan hasil perbandingan akurasi pada data uji, metode SES merupakan metode terbaik karena menghasilkan nilai kesalahan peramalan paling kecil dibandingkan metode lainnya.